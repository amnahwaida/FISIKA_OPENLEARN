# Bab 7: Fisika Modern & Relativitas Khusus

[< Kembali ke Daftar Materi Kelas 12](./README.md) | [Ke Beranda](../README.md)

---

## Daftar Isi
1. [Postulat Einstein & Transformasi Lorentz](#1-postulat-einstein--transformasi-lorentz)
2. [Dilatasi Waktu (Time Dilation)](#2-dilatasi-waktu-time-dilation)
3. [Kontraksi Panjang (Length Contraction)](#3-kontraksi-panjang-length-contraction)
4. [Massa & Energi Relativistik ($E = mc^2$)](#4-massa--energi-relativistik-e--mc2)
5. [Efek Fotolistrik & Dualisme Cahaya](#5-efek-fotolistrik--dualisme-cahaya)
6. [Efek Compton & Momentum Foton](#6-efek-compton--momentum-foton)
7. [Hipotesis De Broglie (Gelombang Materi)](#7-hipotesis-de-broglie-gelombang-materi)
8. [Ketidakpastian Heisenberg & Kuantum](#8-ketidakpastian-heisenberg--kuantum)
9. [Radiasi Benda Hitam & Konstanta Planck](#9-radiasi-benda-hitam--konstanta-planck)

---

## 1. Postulat Einstein & Transformasi Lorentz

Mekanika Newton gagal menjelaskan fenomena pada kecepatan mendekati cahaya. Albert Einstein (1905) merombak fisika dengan dua postulat:
1. **Hukum Fisika adalah sama** untuk semua kerangka acuan inersia.
2. **Kecepatan cahaya ($c$) adalah konstan** ($3 \times 10^8 \text{ m/s}$) bagi semua pengamat, tidak peduli seberapa cepat pengamat atau sumber cahaya bergerak.

**Faktor Lorentz ($\gamma$):** Besaran yang menentukan seberapa besar efek relativitas muncul.
$$ \gamma = \frac{1}{\sqrt{1 - \frac{v^2}{c^2}}} $$

---

## 2. Dilatasi Waktu (Time Dilation)

Waktu bukan lagi besaran mutlak. Bagi pengamat yang bergerak sangat cepat ($v \to c$), waktu akan terasa berjalan lebih lambat dibandingkan pengamat yang diam.
$$ \Delta t = \gamma \cdot \Delta t_0 $$
- $\Delta t_0$: Waktu diam (waktu asli).
- $\Delta t$: Waktu bergerak (waktu yang melambat).

---

## 3. Kontraksi Panjang (Length Contraction)

Benda yang bergerak mendekati kecepatan cahaya akan tampak memendek searah dengan arah geraknya bagi pengamat diam.
$$ L = \frac{L_0}{\gamma} = L_0 \sqrt{1 - \frac{v^2}{c^2}} $$

---

## 4. Massa & Energi Relativistik ($E = mc^2$)

Massa benda meningkat seiring bertambahnya kecepatan.
$$ m = \gamma \cdot m_0 $$
**Energi Total:**
$$ E = mc^2 = \gamma \cdot m_0 c^2 $$
**Energi Kinetik Relativistik ($E_k$):**
$$ E_k = E_{total} - E_{diam} = (\gamma - 1) m_0 c^2 $$

---

## 5. Efek Fotolistrik & Dualisme Cahaya

Cahaya tidak hanya berupa gelombang, tetapi juga paket energi diskrit yang disebut **Foton**.
- **Peristiwa:** Lepasnya elektron dari logam akibat disinari cahaya frekuensi tinggi.
- **Syarat:** Frekuensi cahaya ($f$) > Frekuensi ambang ($f_0$).
$$ E_{foton} = W_0 + E_k \implies h \cdot f = h \cdot f_0 + E_k $$

---

## 6. Efek Compton & Momentum Foton

Arthur Compton membuktikan bahwa foton memiliki momentum dan bisa "menabrak" elektron layaknya bola biliar.
$$ P_{foton} = \frac{h}{\lambda} $$
**Perubahan Panjang Gelombang ($\Delta \lambda$):**
$$ \Delta \lambda = \lambda' - \lambda = \frac{h}{m_e c} (1 - \cos \theta) $$

---

## 7. Hipotesis De Broglie (Gelombang Materi)

Jika gelombang (cahaya) bisa bersifat partikel, maka partikel (materi) juga harus bisa bersifat gelombang.
$$ \lambda_{materi} = \frac{h}{p} = \frac{h}{m \cdot v} $$
Inilah dasar dari pembuatan **Mikroskop Elektron** yang memiliki resolusi jauh lebih tinggi daripada mikroskop cahaya.

---

## 8. Ketidakpastian Heisenberg & Kuantum

Kita tidak bisa mengukur posisi ($x$) dan momentum ($p$) suatu partikel secara bersamaan dengan akurasi tak hingga.
$$ \Delta x \cdot \Delta p \geq \frac{h}{4\pi} $$
Ini bukan masalah alat ukur, melainkan sifat fundamental alam semesta pada skala atom.

---

## 9. Radiasi Benda Hitam & Konstanta Planck

Lahirnya Fisika Kuantum bermula dari Max Planck yang memecahkan "Bencana Ultraviolet" dengan mengasumsikan energi terpancar dalam paket-paket (*Quanta*).
$$ E = n \cdot h \cdot f $$
- $h = 6,63 \times 10^{-34} \text{ Js}$ (Konstanta Planck).

---

## Lihat Juga
- [Materi Kelas 10 & 11](../README.md)
- [Latihan Soal Bab 7 Fisika Modern](latihan-soal/README.md)
